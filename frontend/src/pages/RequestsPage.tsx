        ...
        <tbody className="divide-y divide-border">
          {requests.map((r) => (
            <tr key={r.id} className="hover:bg-muted/20">
              <td className="px-4 py-2.5 font-mono-data font-medium text-foreground">
                {r.id}
              </td>
              <td className="px-4 py-2.5 font-mono-data text-[11px] text-muted-foreground uppercase">
                {r.type}
              </td>
              ...
              <td className="px-4 py-2.5">
                <span
                  className={`px-2 py-0.5 text-[11px] font-medium rounded-sm ${statusColors[r.status]}`}
                >
                  {r.status.replace("_", " ")}
                </span>
              </td>
              <td className="px-4 py-2.5 font-mono-data text-muted-foreground">
                {r.sla_remaining}
              </td>
              <td className="px-4 py-2.5 text-muted-foreground uppercase text-[11px]">
                {r.handler}
              </td>
              <td className="px-4 py-2.5 font-mono-data text-muted-foreground">
                {r.created}
...
const fetchRequests = async () => {
  const baseUrl =
    ...
    "http://127.0.0.1:8000";
  const res = await fetch(`${baseUrl}/cloud/requests`);
    ...
    path === "/requests/delete"
      ...
      : path === "/requests/access"
        ...
        : path === "/requests/correction"
          ? requests.filter((r) => r.type === "correction")
          : requests;
    ...
    <div className="p-4 lg:p-6 space-y-6">
      <div>
        ...
        <p className="text-[13px] text-muted-foreground mt-0.5">
          Data Subject Request handler — Track lifecycle and SLA
        </p>
      </div>
      <SubtabNav />
      {/* <button
            ...
            <tbody className="divide-y divide-border">
              {queueItems.map((q) => (
                <tr key={q.id} className="hover:bg-muted/20">
                  ...
                  <td className="px-4 py-2.5 font-mono-data text-primary">
                    {q.request}
                  </td>
                  ...
                  <td className="px-4 py-2.5">
                    <span
                      className={`px-2 py-0.5 text-[11px] font-medium rounded-sm ${statusColors[q.status]}`}
                    >
                      {q.status.replace("_", " ")}
                    </span>
                  </td>
                  <td className="px-4 py-2.5">
                    <span
                      className={`px-2 py-0.5 text-[11px] font-medium rounded-sm uppercase ${
                        q.priority === "critical"
